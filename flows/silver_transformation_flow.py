...
def get_active_procs(client_id, conn):
    with conn.cursor() as cur:
        ...
        if not rows:
            raise ValueError(
                f"Tidak ditemukan procedure aktif untuk client_id {client_id}"
            )
...
def update_batch_file_with_procs(file_path, proc_names):
    ...
    if "transformation_procedure" not in data:
        ...
        i = 1
        while f"transformation_procedure_reprocess{i}" in data:
            i += 1
        data[f"transformation_procedure_reprocess{i}"] = proc_names
...
# Main Prefect Flow
# -----------------------------
@flow(name="Silver Clean Transform Flow")
async def silver_clean_transform_flow(client_schema: str):
    """Execute silver clean transform procedures for client schema"""
    logger = get_run_logger()
    ...
    try:
        logger.info(f"Starting silver clean transform for {client_schema}")
        print(f"Starting silver clean transform for {client_schema}")
        ...
        # ===== CUSTOM NAME UPDATE =====
        custom_name = f"{client_schema}-{batch_id}-silver-clean-transform"
        ...
        try:
            ...
            logger.info(
                f"Procedure aktif untuk client '{client_schema}' (client_id={client_id}): {proc_names}"
            )
            print(
                f"Procedure aktif untuk client '{client_schema}' (client_id={client_id}): {proc_names}"
            )
        except Exception as e:
            error_msg = f"Error getting active procedures: {e}"
            logger.error(error_msg)
            print(f"[FAIL] {error_msg}")
            traceback.print_exc()
            try:
        ...
        for proc_name in proc_names:
            logger.info(f"Executing procedure: {proc_name}")
            is_success, error_message = run_procedure(
                proc_name, client_schema, batch_id, client_id
            )
            if not is_success:
                all_success = False
                error_messages.append(f"{proc_name} gagal: {error_message}")
                logger.error(f"Procedure {proc_name} failed: {error_message}")
        ...
        if all_success:
            try:
                ...
                logger.info("Silver clean transform completed successfully")
                print("[OK] Silver clean transform completed successfully")
                conn.close()
                return True
            except Exception as e:
            ...
            try:
                ...
                move_file(file_path, client_schema, "FAILED")
                logger.error(f"Silver clean transform failed: {final_error_msg}")
                print(f"[FAIL] Silver clean transform failed: {final_error_msg}")
                conn.close()
                return False
            except Exception as e:
        ...
        end_time = datetime.now()
        error_msg = f"Unexpected error in silver clean transform: {e}"
        logger.error(error_msg)
        print(f"[FAIL] {error_msg}")
        traceback.print_exc()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python silver_clean_transform_flow.py <client_schema>")
        sys.exit(2)
    ...
    result = asyncio.run(silver_clean_transform_flow(client_schema))
